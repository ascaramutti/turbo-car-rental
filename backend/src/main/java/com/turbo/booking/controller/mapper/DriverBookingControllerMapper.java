...
public interface DriverBookingControllerMapper {
    ...
    /** Adds the platform markup to an owner's net hourly rate so drivers see the price they will pay. */
    @Named("applyDriverMarkup")
    default BigDecimal applyDriverMarkup(BigDecimal netHourlyRate) {
        return PricingConstants.applyMarkup(netHourlyRate);
    }
    ...
    @Mapping(source = "vehicle.hourlyRate", target = "vehicleHourlyRate", qualifiedByName = "applyDriverMarkup")
    ...
    @Mapping(source = "vehicle.hourlyRate", target = "hourlyRate", qualifiedByName = "applyDriverMarkup")
    ...
    @Mapping(source = "ownerRating", target = "ownerRating")
