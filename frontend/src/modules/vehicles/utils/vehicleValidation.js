...
export function validateActivateForm(data) {
  ...
  // Lat/lng come from the map picker, never typed by hand. Surface a single
  // human-readable error so the owner is told to interact with the map.
  const noLatitude = data.latitude === '' || data.latitude === null || data.latitude === undefined;
  const noLongitude = data.longitude === '' || data.longitude === null || data.longitude === undefined;
  if (noLatitude || noLongitude) {
    errors.pickupPoint = 'Click the map to set the vehicle pickup point';
  }
