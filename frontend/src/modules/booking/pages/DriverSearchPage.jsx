...
/** Default search location used when geolocation is unavailable or denied (Douglas College, New Westminster). */
...
function sortVehicles(vehicles, sortKey) {
  ...
  // 'closest' — the backend returns results already ordered by ascending distance from the search point.
  return copy;
}
...
export default function DriverSearchPage() {
  ...
  useEffect(() => {
    // Filters are already seeded with the default location, so on failure or no support
    // we silently keep the default — no error toast to avoid spooking the user on first load.
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        // Silent fallback — keep the default location already in state.
      }
    );
  }, []);
