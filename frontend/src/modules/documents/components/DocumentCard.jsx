...
import { uploadDocument, reuploadDocument, getDocumentViewUrl, getDocumentDownloadUrl } from '../api/documentApi';
...
/**
 * Displays upload card for a single document type or shows status if uploaded.
 * @param {string} documentType - DRIVERS_LICENSE or STUDY_PERMIT
 * @param {Object|null} document - Uploaded document object or null
 * @param {Function} onUpdate - Callback triggered after successful upload/reupload
 */
export default function DocumentCard({ documentType, document, onUpdate }) {
  ...
  const handleFileSelected = async (e, isReupload) => {
    ...
    try {
      if (isReupload) {
        await reuploadDocument(document.documentId, file);
      } else {
  ...
  const handleDownload = async () => {
    ...
    try {
      await downloadFile(getDocumentDownloadUrl(document.documentId), document.fileName);
    } catch (err) {
      toast.error(extractErrorMessage(err));
    } finally {
  ...
  if (!document) {
    return (
      <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-accent-orange transition-colors">
        ...
        <p className="text-xs text-text-gray mb-4">PDF, JPG, or PNG (max {FILE_CONSTRAINTS.MAX_SIZE_MB}MB)</p>
  ...
  return (
    <div className={`border-2 rounded-xl p-6 ${config.border} ${config.bg}`}>
      ...
      {showPreview && (
        <FilePreviewModal
          fileUrl={getDocumentViewUrl(document.documentId)}
